              ...
              >
                <Switch>
                  ...
                  <PrivateRoute
                    exact
                    path="/expenses"
                    component={() => <ExpensesPage auth={auth} />}
                  />
                  <PrivateRoute
                    exact
                    path="/reports"
                    component={() => <FinancialReportsPage auth={auth} />}
                  />
                  <PrivateRoute
                    exact
                    path="/settings"
                    component={() => <SettingsPage auth={auth} />}
                  />
                  <PrivateRoute
                    path="/people/:person_id"
                    component={() => <Person auth={auth} />}
                  />
                </Switch>
              </AppShell>
            ) : (
              <div>
                Hi, {auth.user.data.attributes.name}! Your Planning Center
                Account does not have the required privileges to access this
                site. Please ask an existing administrator for admin access.
              </div>
            ))}
...
function useProvideAuth() {
  ...
  useEffect(() => {
    if (window.location.pathname === "/logout") {
      if (checkCookie("jwt")) {
        console.log("jwt cookie exists, redirecting to home");
      ...
      if (checkCookie("jwt")) {
        ...
        console.log(
          "jwt cookie doesnt exist, using code in this URL to ping pco for auth token"
        );
        ...
        if (code) {
            ...
            .then((token) => {
              ...
              getCurrentUserData().then((data) => {
                if (data) {
                  ...
                  console.log(
                    "code but no data(wrong code used), attempting step 1"
                  );
                  signin();
                }
              });
          ...
          console.log(
            "code doesnt exist, starting from step 1, redirecting to oauth."
          );
          signin();
        }
      }
