...
export class People extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        "https://api.planningcenteronline.com/people/v2/people?per_page=1000",
        {
          headers: {
    ...
    if (e.target.value !== "") {
      people &&
        people.data.forEach(function (person) {
          ...
          var first_name =
            person.attributes.first_name != null
              ? person.attributes.first_name
                  .normalize("NFD")
                  .replace(/[\u0300-\u036f]/g, "")
              : null;
          var middle_name =
            person.attributes.middle_name != null
              ? person.attributes.middle_name
                  .normalize("NFD")
                  .replace(/[\u0300-\u036f]/g, "")
              : null;
          var last_name =
            person.attributes.last_name != null
          ...
          if (
            (first_name !== null &&
              first_name.toLowerCase().indexOf(e.target.value) !== -1) ||
            (middle_name !== null &&
              middle_name.toLowerCase().indexOf(e.target.value) !== -1) ||
            (last_name !== null &&
              last_name.toLowerCase().indexOf(e.target.value) !== -1)
          ) {
  ...
  render() {
    return (
      <div>
        {this.state.reshapedPeople && (
          <AutocompleteSearch
            data={this.state.reshapedPeople}
            enableRedirect={true}
          />
        )}
